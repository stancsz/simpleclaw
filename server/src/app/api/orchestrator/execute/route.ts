...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!sessionId || typeof sessionId !== 'string') {
            return Response.json({ error: 'Missing or invalid "session_id" field in request body.' }, { status: 400 });
        }
        ...
        executeSwarmManifest(manifest, sessionId, dbClient).catch((err) => {
            console.error('Error in executeSwarmManifest:', err);
        });
        ...
        console.error('Error in orchestrator execute API route:', error);
